  ...
  render() {
   return (
     <div className={classes.InvoicePrint}>
       ...
       <div>
         <strong>Заказчик: {this.props.current.client}</strong>
       </div>
       <div>
         <strong>Плательщик: {this.props.current.client}</strong>
       </div>
       <br />
       <span className='mt-5'>Судно: {this.props.current.vessel}</span>
       <span className='ml-5'>Рейс: {this.props.current.voyage}</span>
       {this.props.current.bl && <div>Коносамент: {this.props.current.bl}</div>}
